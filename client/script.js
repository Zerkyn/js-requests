...
sayHelloButton.addEventListener('mouseover', blackBtn)
...
sayHelloButton.addEventListener('mouseout', whiteBtn)
...
const ohMy = () => {
    ...
    axios.get('http://localhost:3000/animals').then(res => {
        for (let i = 0; i < res.data.length; i++) {
            let text = document.createElement('p').textContent = res.data[i]
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/name').then(res => {
...
// CODE HERE
const queryReq = () => {
    axios.get(`http://localhost:3000/query-test/?name=timmay&lastname=bob`).then(res => {
        console.log(res.data)
    })
}
document.querySelector('#query-button').addEventListener('click', queryReq)
